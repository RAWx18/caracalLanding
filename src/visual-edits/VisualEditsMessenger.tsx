...
export const CHANNEL = "CARACAL_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "caracal_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "caracal_focused_element" as const;
...
const postMessageDedup = (data: any) => {
  try {
    ...
    if (key === _caracalLastMsg) return; // identical – drop
    _caracalLastMsg = key;
  } catch {
...
// Helper to parse data-caracal-id to extract file path, line, and column
const parseCaracalId = (
  caracalID: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = caracalID.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-caracal-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-caracal-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the caracal ID from the element to ensure we're working with the right one
    const caracalID = element.getAttribute("data-caracal-id");
    if (!caracalID) return;
    ...
    if (newText !== oldText) {
      const parsed = parseCaracalId(caracalID);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same caracal ID
    const allMatchingElements = document.querySelectorAll(
      `[data-caracal-id="${caracalID}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(caracalID) || {};
    appliedStylesRef.current.set(caracalID, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(caracalID);
  ...
  const flushImageSrcChange = () => {
    ...
    const caracalID = imgElement.getAttribute("data-caracal-id");
    if (!caracalID) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "CARACAL_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same caracal ID
        const allMatchingElements = document.querySelectorAll(
          `[data-caracal-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "CARACAL_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-caracal-id
        const caracalID = element.getAttribute("data-caracal-id");
        if (caracalID) {
          const parsed = parseCaracalId(caracalID);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-caracal-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-caracal-id");
        ...
        const tagName =
          hit.getAttribute("data-caracal-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same caracal ID
        const allMatchingElements = document.querySelectorAll(
          `[data-caracal-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-caracal-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-caracal-id]");
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same caracal ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-caracal-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data-caracal-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same caracal ID
        const allMatchingElements = document.querySelectorAll(
          `[data-caracal-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-caracal-name") ||
